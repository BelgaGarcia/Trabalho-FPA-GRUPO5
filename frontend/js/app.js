...
(function () {
  ...
  function normalizarResultado(dados) {
    var d = dados || {};
    var subsequencias = d.subsequencias || d.padroes || [];
    ...
    return {
      helena: d.helena || "",
      marcus: d.marcus || "",
      comprimento: comprimento !== undefined && comprimento !== null ? comprimento : "-",
      quantidade: d.quantidade !== undefined && d.quantidade !== null
        ? d.quantidade
        : subsequencias.length,
      subsequencias: Array.isArray(subsequencias) ? subsequencias : [],
  ...
  function renderResultado(dados, numero) {
    var resultado = normalizarResultado(dados);
    ...
    par.appendChild(criarParInfo("Helena", resultado.helena));
    par.appendChild(criarParInfo("Marcus", resultado.marcus));
    head.appendChild(par);

    if (resultado.algoritmo) {
      var algoritmo = document.createElement("p");
      algoritmo.className = "result__algo";
      algoritmo.textContent = resultado.algoritmo;
      head.appendChild(algoritmo);
    }
    ...
    stats.appendChild(
      criarStat("Comprimento máximo", String(resultado.comprimento))
    );
    stats.appendChild(
      criarStat("Subsequências", String(resultado.quantidade))
    );
    card.appendChild(stats);

    // Requisito B: subsequencias.
    card.appendChild(criarSubsequencias(resultado.subsequencias));

    // Requisito C: tabela DP.
    card.appendChild(renderTabelaDP(resultado.tabela, resultado.helena, resultado.marcus));
